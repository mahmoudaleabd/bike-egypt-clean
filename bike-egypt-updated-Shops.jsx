...
import { MapPin, Phone, Star, Clock, Wrench, Package, Search, Flame, MapPinIcon } from 'lucide-react'
...
function Shops() {
  ...
  // Simulate loading state on mount and filter changes
  useEffect(() => {
    setIsLoading(true)
  ...
  }, [filter, searchTerm, setSearchParams])
    ...
    <div className="min-h-screen bg-gradient-to-b from-slate-950 to-slate-900/50 py-8 md:py-12">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-12 animate-fade-in">
          <h1 className="text-3xl md:text-4xl lg:text-5xl font-display font-bold gradient-text mb-4">
            Bike Shops & Services
          </h1>
        ...
        {isLoading ? (
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            {filteredShops.map((shop, index) => (
              <div
                key={shop.id}
                className="card-hover group overflow-hidden animate-slide-up hover:shadow-lg hover:-translate-y-1 transition-all duration-300"
              ...
              >
                ...
                {/* Content */}
                <div>
                  <h3 className="text-lg md:text-xl font-bold text-slate-100 mb-3 group-hover:text-primary-300 transition-colors duration-300">
                    {shop.name}
                  </h3>
          ...
          <EmptyState type="shops" searchTerm={searchTerm} onReset={handleReset} />
        )}
      </div>
    </div>
  )
